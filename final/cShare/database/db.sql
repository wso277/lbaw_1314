...
CREATE OR REPLACE FUNCTION deleteEditor()
  RETURNS TRIGGER AS $deleteEditor$
BEGIN
  DELETE FROM Amizade
  WHERE amigo1 = OLD.username OR amigo2 = OLD.username;
...
CREATE OR REPLACE FUNCTION deleteCategoria()
  RETURNS TRIGGER AS $deleteCategoria$
BEGIN
  DELETE FROM Noticia
  WHERE idNoticia IN (SELECT
                        idNoticia
                      FROM NoticiaCategoria
                      WHERE nome = OLD.nome);
  RETURN OLD;
END;
...
CREATE OR REPLACE FUNCTION updateLikes()
  RETURNS TRIGGER AS $updateLikes$
BEGIN
  UPDATE Noticia
  SET likes = (SELECT
                 SUM(avaliacao)
...
CREATE TRIGGER updateLike AFTER UPDATE ON AvaliarNoticia FOR EACH ROW EXECUTE PROCEDURE updateLikes();
