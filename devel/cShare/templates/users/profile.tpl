...
{else}
    ...
    <script type="text/javascript" src="{$BASE_URL}javascript/friends_redirect.js"></script>
    ...
    <div class="panelx">
        ...
        <div class="name">
            <small>{$user[0].nome}, {$user[0].localidade}</small>
        </div>
    </div>
        ...
        <div class="btn-group btn-group-sm">
            {if ($USERNAME != $user[0].username)}
                <a class="btn btn-success"
                   href="{$BASE_URL}pages/users/send_message.php?sender={$USERNAME}&receiver={$user[0].username}">
                    <i class="glyphicon glyphicon-envelope"></i>&nbsp;Send Message
                </a>
            {else}
                <a href="{$BASE_URL}pages/users/send_message.php?sender={$USERNAME}&receiver=" class="btn btn-success">
                    <i class="glyphicon glyphicon-envelope"></i>&nbsp;Send Message
                </a>
            {/if}
        </div>
    ...
    <ul class="nav nav-tabs" id="myTab">
        <li id="inf" class="active"><a href="#info" data-toggle="tab"><i class="glyphicon glyphicon-list-alt"></i>
                Info</a></li>
        {if ($USERNAME == $user[0].username)}
        ...
        {if ($USERNAME == $user[0].username || $isFriend)}
            <li id="fri"><a href="#f" id="friends" data-toggle="tab"><i class="glyphicon glyphicon-list"></i> Friends</a>
            </li>
        {/if}
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="info">
            <div style="padding-left:3em;padding-top:1em">
                ...
                <div style="display:inline" id="username">{$user[0].username}</div>
            </div>
        </div>
                    ...
                    <i class="glyphicon glyphicon-filter"></i>
                        ...
                        <table class="table table-hover" id="dev-table">
                            ...
                            {foreach $sentMessages as $msg}
                                <tr>
                                    <td>
                                        <a href="{$BASE_URL}pages/users/see_message.php?id={$msg.idmensagem}">{$msg.titulo}</a>
                                    </td>
                                    <td>
        ...
        <div class="tab-pane" id="posts">
            {foreach $posts as $post}
                <a href="{$BASE_URL}pages/content/content.php?id={$post.idnoticia}">
                    <div class="well well-sm" style="margin:0px;background-color:#222;border-color:#222">{$post.titulo}
                        <span class="pull-right"><i class="glyphicon glyphicon-time"></i> {$post.data_post} </span>
                    </div>
                </a>
            {/foreach}
        </div>
                ...
                {foreach $friends as $friend}
                    <div class="row" style="padding-left:7em;padding-top:0.7em">
                        <div class="user_image">
                            <img src="{$friend.photo}" width="64em" height="64em" style="float:left"/>
                        </div>
                        <div class="user_name" style="padding-left: 5em; padding-top: 0.7em">
                            ...
                            {if ($USERNAME == $user[0].username)}
                                <div class="btn-group btn-group-sm">
                                    <a class="btn btn-success" id="remove_friends" name="{$friend.username}">
                                        <i class="glyphicon glyphicon-heart-empty"></i>&nbsp;Remove Friend
                                    </a>
                                </div>
                            {/if}
