...
<body>
	...
	<div class="content-text">
		<div class="text">
			<h3>Link para a noticia: {foreach $links as $link}<a href="{$link.href}">{$link.homelink}</a>&nbsp;{/foreach}</h3>
			{$content.conteudo}
		</div>
		...
		<form action="{$BASE_URL}ations/content/comment.php" method="post">
			<div class="row" style="text-align:right;margin-left:4em">
				<div class="col-lg-6">
					...
					<textarea class="form-control" style="resize: none" placeholder="Leave a comment..." rows="4"></textarea>
				</div>
			</div>
		</form>
		...
		<div class="comments-container">
			...
			<div class="comment text">
				<div class="avatar">
					<img class="avatar-image" src="{$comment.photo.fotografia}" width="48em" height="48em" style="float:left"/> 
				</div>
				<div class="comment-author">
					<b>{$comment.username}</b> &nbsp;{$comment.likes} {if $comment.likes > 1}points{else if $comment.likes != NULL}point{/if}{if $comment.likes == NULL}0 points{/if}
				</div>
				<div class="comment-text">
					{$comment.content}
				</div>
						...
						<div class="modal-dialog">
							<div class="panel panel-primary" style="border-color:#428bca">
								<div class="panel-heading" style="background-color:#428bca;border-color:#428bca">
									...
									<h4 class="panel-title" id="contactLabel"><span class="glyphicon glyphicon-wrench"></span> Edit comment</h4>
								</div>
								<form action="#" method="post" accept-charset="utf-8">
									<div class="modal-body" style="padding: 5px;">
										<div class="row">
											<div class="col-lg-12 col-md-12 col-sm-12">
												<textarea style="resize:vertical;" class="form-control" placeholder="New Comment..." rows="6" name="comment" required></textarea>
											</div>
										</div>
									</div>  
									<div class="panel-footer" style="margin-bottom:-14px;">
										...
										<button style="float: right;" type="button" class="btn btn-primary btn-close" data-dismiss="modal">Close</button>
									</div>
								</div>
							</div>
						</div>
